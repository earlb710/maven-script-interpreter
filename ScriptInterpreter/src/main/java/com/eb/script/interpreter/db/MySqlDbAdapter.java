...
/**
 * MySQL JDBC implementation of Interpreter.DbAdapter.
 *
 * Connection spec accepted by connect(spec):
 *   1) String  -> treated as full JDBC URL, e.g. "jdbc:mysql://host:3306/database"
 ...
 */
public final class MySqlDbAdapter implements DbAdapter {
    ...
    public DbConnection connect(Object spec) throws Exception {
        ...
        return new MySqlDbConnection(conn, cfg.defaultFetchSize);
    }
    ...
    private static final class ConnectConfig {
        ...
        static ConnectConfig from(Object spec) {
            if (spec instanceof String s) {
                ...
                // Build URL if absent: jdbc:mysql://host:port/database
                if (url == null) {
                    if (host != null && database != null) {
                        int p = (port == null ? 3306 : port);
                        url = "jdbc:mysql://" + host + ":" + p + "/" + database;
                    } else {
    ...
    // ------------------- MySqlDbConnection -------------------
    private static final class MySqlDbConnection implements DbConnection {
        private final Connection conn;
        private final Integer defaultFetchSize;

        MySqlDbConnection(Connection conn, Integer fetchSize) {
            this.conn = conn;
            this.defaultFetchSize = fetchSize;
        }
        ...
        public DbCursor openCursor(String sql,
            ...
            return new MySqlDbCursor(ps, rs);
        }
    ...
    // ------------------- MySqlDbCursor -------------------
    private static final class MySqlDbCursor implements DbCursor {
        ...
        MySqlDbCursor(PreparedStatement ps, ResultSet rs) {
            this.ps = ps;
            this.rs = rs;
        }
        ...
        public boolean hasNext() throws Exception {
            if (!peeked) {
                ...
                if (hasRow) {
                    ...
                    cachedRow = MySqlDbConnection.readRow(rs, md, md.getColumnCount());
                }
                peeked = true;
            }
            return hasRow;
        ...
        public Map<String, Object> next() throws Exception {
            if (!peeked) {
                if (rs.next()) {
                    ResultSetMetaData md = rs.getMetaData();
                    return MySqlDbConnection.readRow(rs, md, md.getColumnCount());
                }
                return null;
            }
