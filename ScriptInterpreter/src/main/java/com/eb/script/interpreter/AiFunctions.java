 ...
 *
 * Configuration (system property OR env var):
 *   - ai.chat.url       / AI_CHAT_URL       (default: https://api.openai.com/v1/chat/completions)
 *   - ai.embed.url      / AI_EMBED_URL      (default: https://api.openai.com/v1/embeddings)
 *   - ai.api.key        / AI_API_KEY        (default: none -> no Authorization header)
 *   - ai.chat.model     / AI_CHAT_MODEL     (default: gpt-4o-mini)
 *   - ai.embed.model    / AI_EMBED_MODEL    (default: text-embedding-3-small)
 *   - ai.timeout.ms     / AI_TIMEOUT_MS     (default: 30000)
 *
...
public final class AiFunctions {
    ...
    public static String chatComplete(String system, String user, Integer maxTokens, Double temperature)
        ...
        if (maxTokens != null && maxTokens > 0) req.put("max_tokens", maxTokens);
    ...
    public static double[] embed(String input) throws IOException, InterruptedException {
        ...
        for (int i = 0; i < list.size(); i++) {
            Object v = list.get(i);
            if (v instanceof Number n) out[i] = n.doubleValue();
        }
        return out;
    }

    /**
     * Lightweight classifier using a zero/few-shot prompt.
     * Returns the predicted label as plain text.
     */
